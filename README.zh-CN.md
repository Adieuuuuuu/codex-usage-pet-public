...
它还有一个很实际的优势：电脑端会直接只读读取本机 Codex Desktop 的状态，
并且会去 Codex 原本的桌宠目录（`~/.codex/pets`）发现标准 Hatch Pet 桌宠包。
你已经在 Codex 里使用的桌宠可以直接沿用，不需要在第二个应用里重新制作一套。
有效的本机桌宠会优先于内置默认桌宠，因此第一次上手更接近“启动就能用”：
不用手动导出状态，不用转换素材，也不用复制粘贴中间数据。

这个项目最初也来自远程控制和状态监控之间的一个实际区别。官方 Codex Remote 适合
发起任务，但网络往返会让体验变慢，而且它不是电脑本机前端预览、IDE、终端和其他
桌面上下文的窗口。真正需要操作电脑时，直接使用远程桌面软件会更直观、更清晰，也
更快捷。Usage Pet 是对这套方式的补充：它不替代远程控制，也不把本机桌面画面搬到
手机上；它只读取本机 Codex 状态，并及时推送任务进度。远程桌面负责“直接操作电脑”，
Usage Pet 负责“告诉你任务进展”，不需要再依赖 Codex Remote 做持续监控。
...
- 与电脑端一致的用量胶囊和任务信息层级；
- 通知保留三条完整任务，并可通过“查看全部”进入 App 滚动查看完整同步列表；
